...
import { ArrowLeft, ChevronUp, Plus, Save, Trash2, Upload, Settings, ToggleLeft } from "lucide-react";
...
const ZONE_ACTION_BUTTONS = [
  ...
  { id: 'mode', label: 'Mode', icon: ToggleLeft, color: '#C7FFBF', isToggle: true },
];
  ...
  return (
    ...
    >
      ...
      >
        <button
          onClick={onBack}
          className="p-2 hover:bg-white/5 rounded transition-all group"
        >
          <ArrowLeft size={18} className="text-[#959595] group-hover:text-[#C7FFBF] transition-colors" />
        </button>
      </motion.div>

      {/* Zone Type Badge - Compact */}
      <motion.div
        initial={{ opacity: 0, y: -10 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.15 }}
        className="flex-none mt-2 mx-[9px]"
      >
        <div className="px-2.5 py-1.5 bg-[#1A1A1A] rounded-lg border border-[#2A2A2A]">
          <p className="text-[8px] font-bold uppercase tracking-wider text-[#606060] mb-0.5">Mode</p>
          <p className="text-[10px] font-medium text-white">
            {zoneMode === 'designated' ? 'Designated' : 'Street'}
          </p>
        </div>
      </motion.div>
      ...
      <div className="flex-none">
        ...
        >
          {/* Collapsible Camera Actions Section */}
          <AnimatePresence>
            {showActionsMenu && (
              <motion.div

...
export function TopNav({ cameraId }: TopNavProps) {
  return (
    <div className="flex items-center justify-between gap-6 w-full p-6">
      {/* Camera ID - shown when in management mode */}
      {cameraId && (
        <h1 className="text-white text-3xl font-black tracking-tight" style={{ fontFamily: "'Chillax', 'Nunito', 'Inter', sans-serif" }}>
          {cameraId}
        </h1>
      )}
      
      {/* Spacer when not in management mode */}
      {!cameraId && <div />}
      ...
      <div className="flex items-center gap-6">
        <div className="relative group">
          ...
          <input
            ...
            className="bg-[#2A2A2A] text-white text-sm rounded-full pl-10 pr-4 py-2.5 w-[320px] focus:outline-none focus:ring-1 focus:ring-[#C7FFBF] placeholder-[#606060] transition-all shadow-sm"
          />
        </div>
